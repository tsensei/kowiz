(function ( w, d ) {
	...
	function PKAE () {
		...
		q.init = function ( el_id ) {
			...
			// Check for 'url' parameter to preload audio from URL
			if (w.location.href.split('url=')[1]) {
				var audioUrl = decodeURIComponent(w.location.href.split('url=')[1].split('&')[0]);

				// Wait a bit for the UI to initialize, then load the URL
				setTimeout(function() {
					q.engine.LoadURL(audioUrl);
				}, 500);
			}
			// Check for 'local' parameter to load from IndexedDB
			else if (w.location.href.split('local=')[1]) {
